# ARCHIVE LOG

## 移動履歴

### 2025-11-08
以下の旧版リリースノートを `docs/archive/` に退避（配布用リポジトリ整理）。

- RELEASE_NOTES_v1.13.2.md
- RELEASE_NOTES_v1.15.2.md
- RELEASE_NOTES_v1.16.0.md
- RELEASE_NOTES_v1.17.10.md
- RELEASE_NOTES_v1.17.11.md
- RELEASE_NOTES_v1.18.0.md
- RELEASE_NOTES_v1.18.1.md
- RELEASE_NOTES_v1.18.2.md
- RELEASE_NOTES_v1.18.3.md
- RELEASE_NOTES_v1.18.4.md
- RELEASE_NOTES_v1.18.5.md
- RELEASE_NOTES_v1.19.0.md
- RELEASE_NOTES_v1.19.1.md
- RELEASE_NOTES_v1.19.2.md

備考: 当時最新の v1.20.0 は一時的にルート保持。

### 2025-11-25
v2.0.x 系（初期移行期）のリリースノートをアーカイブへ移動。

- RELEASE_NOTES_v2.0.0.md
- RELEASE_NOTES_v2.0.1.md
- RELEASE_NOTES_v2.0.5.md
- RELEASE_NOTES_v2.0.6.md
- RELEASE_NOTES_v2.0.7.md
- RELEASE_NOTES_v2.0.8.md

現行維持: 最新安定版 v2.1.8 のみ `docs/` 直下に残置。

### 2025-12-28
v2.1.x〜v2.3.x の旧リリースノートを `docs/archive/release_notes/` に退避（配布用リポジトリ整理）。

- RELEASE_NOTES_v2.1.8.md
- RELEASE_NOTES_v2.1.9.md
- RELEASE_NOTES_v2.1.10.md
- RELEASE_NOTES_v2.1.11.md
- RELEASE_NOTES_v2.1.12.md
- RELEASE_NOTES_v2.1.13.md
- RELEASE_NOTES_v2.1.14.md
- RELEASE_NOTES_v2.1.15.md
- RELEASE_NOTES_v2.1.16.md
- RELEASE_NOTES_v2.1.24.md
- RELEASE_NOTES_v2.2.0.md
- RELEASE_NOTES_v2.2.1.md
- RELEASE_NOTES_v2.2.2.md
- RELEASE_NOTES_v2.2.3.md
- RELEASE_NOTES_v2.2.4.md
- RELEASE_NOTES_v2.2.5.md
- RELEASE_NOTES_v2.2.6.md
- RELEASE_NOTES_v2.2.7.md
- RELEASE_NOTES_v2.2.8.md
- RELEASE_NOTES_v2.2.9.md
- RELEASE_NOTES_v2.3.0.md
- RELEASE_NOTES_v2.3.1.md
- RELEASE_NOTES_v2.3.2.md
- RELEASE_NOTES_v2.3.3.md
- RELEASE_NOTES_v2.3.4.md
- RELEASE_NOTES_v2.3.6.md
- RELEASE_NOTES_v2.3.7.md
- RELEASE_NOTES_v2.3.8.md
- RELEASE_NOTES_v2.3.9.md

現行維持: 最新安定版 v2.3.10 のみ `docs/` 直下に残置。

### 2025-12-29
最新安定版の更新に伴い、旧最新版のリリースノートを `docs/archive/release_notes/` に退避。

- RELEASE_NOTES_v2.3.10.md
- RELEASE_NOTES_v2.3.11.md

- RELEASE_NOTES_v2.3.12.md

現行維持: 最新安定版 v2.3.13 のみ `docs/` 直下に残置。

### 2025-12-30
最新安定版の更新に伴い、旧最新版のリリースノートを `docs/archive/release_notes/` に退避。

- RELEASE_NOTES_v2.3.13.md

現行維持: 最新安定版 v2.3.14 のみ `docs/` 直下に残置。

### 2025-12-31
最新安定版の更新に伴い、旧最新版のリリースノートを `docs/archive/release_notes/` に退避。

- RELEASE_NOTES_v2.3.14.md

現行維持: 最新安定版 v2.3.15 のみ `docs/` 直下に残置。

### 2025-12-31
最新安定版の更新に伴い、旧最新版のリリースノートを `docs/archive/release_notes/` に退避。

- RELEASE_NOTES_v2.3.15.md

現行維持: 最新安定版 v2.3.16 のみ `docs/` 直下に残置。

### 2026-01-01
最新安定版の更新に伴い、旧最新版のリリースノートを `docs/archive/release_notes/` に退避。

- RELEASE_NOTES_v2.3.16.md

現行維持: 最新安定版 v2.4.0 のみ `docs/` 直下に残置。
