...
## v2.5.63 (2026-06-21) - 配布物整合 + 報告書変換レジューム回復改善

### 📦 配布物

- インストーラを v2.5.63 に更新し、latest.json と SHA256（checksums.sha256）を整合。

### 🧩 利用者向けの主な反映内容

- 報告書変換の途中で壊れた `*.tmp.xlsx` や不整合な `convert_progress.json` が残っていても、安全に破棄して先頭から再開しやすく改善。
- チャンク保存を一時ファイル経由の原子的保存へ見直し、途中保存ファイルの破損リスクを低減。
- 報告書の一括処理で再開時の失敗を起こしにくくし、継続実行の安定性を向上。

### 📚 ドキュメント

- VERSION.txt / README / CHANGELOG / リリースノート等の表記を v2.5.63 に統一。
- docs/RELEASE_NOTES_v2.5.63.md を追加。

## v2.5.61 (2026-05-19) - 配布物整合 + データポータル書誌情報JSON匿名化の user要素除去改善 / 匿名化出力安定化

### 📦 配布物

- インストーラを v2.5.61 に更新し、latest.json と SHA256（checksums.sha256）を整合。

### 🧩 利用者向けの主な反映内容

- データポータル書誌情報 JSON の匿名化で、`included` 内の `type == user` 要素を属性マスクではなく要素ごと除去するよう改善。
- user 要素除去のみが差分だった場合でも匿名化済み JSON を出力し、非開示ファイルの生成漏れを抑制。
- 匿名化処理に関する回帰テストを追加し、データセットアップロード経路での安定性を強化。

### 📚 ドキュメント

- VERSION.txt / README / CHANGELOG / リリースノート等の表記を v2.5.61 に統一。
- docs/RELEASE_NOTES_v2.5.61.md を追加。

## v2.5.60 (2026-05-12) - 配布物整合 + データポータル編集画面ペイロード確認 / チェックボックス挙動強化 / キャッシュ作成日時表示反映

### 📦 配布物

- インストーラを v2.5.60 に更新し、latest.json と SHA256（checksums.sha256）を整合。

### 🧩 利用者向けの主な反映内容

- データポータル編集画面の保存確認ダイアログにペイロードプレビュー表示を追加。編集内容をJSON形式で事前確認でき、送信前の詳細検証が容易に。
- チェックボックス系配列フィールドの全解除時にクリア用送信値（`['']`）へ正規化する挙動を強化。データポータル側のバリデーション精度向上と意図しない値の送信防止。
- データセットアップロード画面のキャッシュ作成日時・更新日時表示を改善。キャッシュの鮮度確認が容易になり、再取得判断が直感的に。

### 📚 ドキュメント

- VERSION.txt / README / CHANGELOG / リリースノート等の表記を v2.5.60 に統一。
- docs/RELEASE_NOTES_v2.5.60.md を追加。

## v2.5.56 (2026-04-22) - 配布物整合 + AI STRUCTURED取得保存先選択 / 既存キャッシュ再利用反映

### 📦 配布物

- インストーラを v2.5.56 に更新し、latest.json と SHA256（checksums.sha256）を整合。

### 🧩 利用者向けの主な反映内容

- AI が追加取得する STRUCTURED ファイルの保存先を temp / dataFiles / dataFilesAI から選べるように改善。
- 既存の dataFiles キャッシュや dataEntry キャッシュを再利用し、再取得の負担を抑えながら AI 補助処理を継続しやすく改善。
- ファイル抽出設定の部分更新時も保存先設定を保持し、設定再保存で値が失われにくいよう整理。

### 📚 ドキュメント

- VERSION.txt / README / CHANGELOG / リリースノート等の表記を v2.5.56 に統一。
- docs/RELEASE_NOTES_v2.5.56.md を追加。

## v2.5.55 (2026-04-21) - 配布物整合 + データポータル managed CSV取得フロー変更対応 / 新旧ヘッダ互換維持 / 一覧表示整理反映

### 📦 配布物

- インストーラを v2.5.55 に更新し、latest.json と SHA256（checksums.sha256）を整合。

### 🧩 利用者向けの主な反映内容

- データポータルの managed CSV 取得で、旧 code/key 方式に加えて最新 filename を使う現行取得フローへ対応し、取得失敗を起こしにくいよう改善。
- managed CSV の新ヘッダ「アトリビュートタグ」「データセットURL」を既存列へ正規化し、旧データとの互換を維持しながら扱いやすく改善。
- 一覧表示で managed 由来の冗長列を抑制し、マージ後の統一値を確認しやすく改善。

### 📚 ドキュメント

- VERSION.txt / README / CHANGELOG / リリースノート等の表記を v2.5.55 に統一。
- docs/RELEASE_NOTES_v2.5.55.md を追加。

## v2.5.54 (2026-04-09) - 配布物整合 + データセット編集候補キャッシュ永続化 / キャッシュ管理可視化反映

### 📦 配布物

- インストーラを v2.5.54 に更新し、latest.json と SHA256（checksums.sha256）を整合。

### 🧩 利用者向けの主な反映内容

- データセット修正画面で既定候補を永続キャッシュとして再利用できるようにし、初回表示後の再選択や再表示を継続しやすく改善。
- 設定画面のキャッシュ管理から、データセット修正候補の永続キャッシュと実行時キャッシュの状態確認・クリアを行いやすく改善。
- 候補再構築中でも検索や選択を続けやすいよう、初期表示時の候補読込経路を整理。

### 📚 ドキュメント

- VERSION.txt / README / CHANGELOG / リリースノート等の表記を v2.5.54 に統一。
- docs/RELEASE_NOTES_v2.5.54.md を追加。

## v2.5.53 (2026-04-08) - 配布物整合 + UI表示リセット / データセット画面の親子関係整理 / 起動時表示安定化反映

### 📦 配布物

- インストーラを v2.5.53 に更新し、latest.json と SHA256（checksums.sha256）を整合。

### 🧩 利用者向けの主な反映内容

- 設定の Misc タブから、メインウィンドウと AI提案ダイアログの保存済み位置・サイズをリセットし、既定レイアウトへ戻せるよう改善。
- データセット開設 / 修正画面で親子関係と進捗ダイアログの生成条件を整理し、非表示状態での不安定な小ウィンドウ表示を抑制。
- 起動時の splash 表示やテーマ切替後の見た目を見直し、初回表示とタイトルバー反映を安定化。

### 📚 ドキュメント

- VERSION.txt / README / CHANGELOG / リリースノート等の表記を v2.5.53 に統一。
- docs/RELEASE_NOTES_v2.5.53.md を追加。

## v2.5.51 (2026-04-06) - 配布物整合 + 更新履歴タブVERSION同期 / サブグループ確認表示改善 / データセット読込と終了時停止安定化反映

### 📦 配布物

- インストーラを v2.5.51 に更新し、latest.json と SHA256（checksums.sha256）を整合。

### 🧩 利用者向けの主な反映内容

- 更新履歴タブを VERSION.txt の履歴をもとに表示する形へ整理し、最新版の概要を安定して確認しやすく改善。
- サブグループ作成・更新時の確認表示で、payload 側の userName を優先しつつ hidden 行の影響を受けにくいよう改善。
- 既存データセット候補の非同期読込中でも、表示済み候補を検索・選択しやすいよう改善。
- アプリ終了時の停止ガードとデータポータル一覧のキャンセル経路を整理し、終了時や中断時の挙動を安定化。

### 📚 ドキュメント

- VERSION.txt / README / CHANGELOG / リリースノート等の表記を v2.5.51 に統一。
- docs/RELEASE_NOTES_v2.5.51.md を追加。

## v2.5.50 (2026-04-06) - 配布物整合 + 主要画面の初回表示安定化 / WebView表示整理 / 一括取得一覧操作改善反映

### 📦 配布物

- インストーラを v2.5.50 に更新し、latest.json と SHA256（checksums.sha256）を整合。

### 🧩 利用者向けの主な反映内容

- ログイン、サブグループ、新規開設2、データ取得2、Basic Info、データポータルなど主要画面で、初回表示時のサイズと位置の復元をより安定化。
- データセット開設 / 通常登録の初回表示調整を整理し、保存済みの表示状態を再利用しやすく改善。
- ログイン関連のメッセージ表示領域を WebView 側へ整理し、不要な枠や空き領域が見えにくいよう改善。
- データ取得2や一括取得（DP）のフィルタや選択列まわりを見直し、一覧操作を継続しやすく改善。

### 📚 ドキュメント

- VERSION.txt / README / CHANGELOG / リリースノート等の表記を v2.5.50 に統一。
- docs/RELEASE_NOTES_v2.5.50.md を追加。

## v2.5.49 (2026-04-05) - 配布物整合 + 主要画面の初回サイズ/位置復元整理 + キャッシュ管理改善反映

### 📦 配布物

- インストーラを v2.5.49 に更新し、latest.json と SHA256（checksums.sha256）を整合。

### 🧩 利用者向けの主な反映内容

- 新規開設 / 新規開設2 / 通常登録などの主要画面で、初回表示時のサイズと中央配置を見直し、開いた直後から扱いやすく改善。
- 旧版で保存された不適切なウィンドウ位置やサイズを 1 回だけ再計算し、以後はタブごとの表示状態を安定して復元するよう改善。
- 設定画面のキャッシュタブを整理し、更新可能なキャッシュ対象だけを明示して再生成しやすく改善。
- キャッシュ更新を非同期で扱い、更新中の状態や進捗を確認しながら他の操作を続けやすく改善。

### 📚 ドキュメント

- VERSION.txt / README / CHANGELOG / リリースノート等の表記を v2.5.49 に統一。
- docs/RELEASE_NOTES_v2.5.49.md を追加。

## v2.5.48 (2026-04-05) - 配布物整合 + 起動直後の応答性改善 / 重い画面の初期表示見直し反映

### 📦 配布物

- インストーラを v2.5.48 に更新し、latest.json と SHA256（checksums.sha256）を整合。

### 🧩 利用者向けの主な反映内容

- 設定、AI、データ取得系の重い画面を初期表示しやすく整理し、起動直後の待ち時間を抑制。
- 新規開設2の既存データセット読込を状態表示付きのバックグラウンド処理へ寄せ、読み込み状況を把握しやすく改善。
- 画面切替や初回ロードのタイミング計測を整理し、継続的な応答性改善に向けた基盤を反映。
- 基本情報タブの初回表示調整や managed 一覧系キャッシュの整理により、表示安定性を改善。

### 📚 ドキュメント

- VERSION.txt / README / CHANGELOG / リリースノート等の表記を v2.5.48 に統一。
- docs/RELEASE_NOTES_v2.5.48.md を追加。

## v2.5.47 (2026-04-04) - 配布物整合 + CSV/XLSX 出力整理 / Excel読込基盤統一反映

### 📦 配布物

- インストーラを v2.5.47 に更新し、latest.json と SHA256（checksums.sha256）を整合。

### 🧩 利用者向けの主な反映内容

- 一覧系の CSV / XLSX 出力を共通化し、表の見た目だけでなく値を保ったまま保存しやすく改善。
- Excel 読み込み処理を openpyxl ベースへ整理し、AI・報告書・設備・設定系の取り込みを安定化。
- 報告書やデータ登録まわりの XLSX 出力経路を整理し、既存の出力機能を維持したまま依存関係を簡素化。
- pandas / xlsxwriter 依存を外し、配布物の構成を見直しつつ既存の CSV / XLSX 利用フローを継続可能に整理。

### 📚 ドキュメント

- VERSION.txt / README / CHANGELOG / リリースノート等の表記を v2.5.47 に統一。
- docs/RELEASE_NOTES_v2.5.47.md を追加。

## v2.5.46 (2026-04-04) - 配布物整合 + Basic Info バックグラウンド取得 / 進捗表示改善反映

### 📦 配布物

- インストーラを v2.5.46 に更新し、latest.json と SHA256（checksums.sha256）を整合。

### 🧩 利用者向けの主な反映内容

- Basic Info の個別データ取得をバックグラウンド化し、タブ操作を続けながら進捗確認と中止操作を行いやすく改善。
- dataset / dataEntry / invoice の取得条件を整理し、新規・欠損・未成功分を優先して不要な再取得を抑制。
- invoice の失敗理由記録により、既知の失敗を繰り返し再試行しないよう改善。
- Basic Info の段階表示と件数ベース進捗を強化し、取得状況を把握しやすく改善。

### 📚 ドキュメント

- VERSION.txt / README / CHANGELOG / リリースノート等の表記を v2.5.46 に統一。
- docs/RELEASE_NOTES_v2.5.46.md を追加。

## v2.5.45 (2026-04-02) - 配布物整合 + 終了時スレッド制御 / create2遅延構築 / データ取得2・一括取得（DP）応答性改善反映

### 📦 配布物

- インストーラを v2.5.45 に更新し、latest.json と SHA256（checksums.sha256）を整合。

### 🧩 利用者向けの主な反映内容

- 終了時に実行中処理を確認し、待機または停止を選びやすく改善。
- 新規開設2の初回表示を遅延構築に変更し、構築中プログレス表示と停止経路を整理。
- データ取得2の初期表示負荷を抑え、一括取得（DP）の検索準備を非同期化して起動直後の操作性を改善。
- 一括取得（DP）の進捗詳細表示を強化し、件数・速度・見込み時間を把握しやすく改善。

### 📚 ドキュメント

- VERSION.txt / README / CHANGELOG / リリースノート等の表記を v2.5.45 に統一。
- docs/RELEASE_NOTES_v2.5.45.md を追加。

## v2.5.44 (2026-04-01) - 配布物整合 + 外部アクセスモニター / TTLキャッシュ / サブグループ完全性チェック / マスタデータ出力反映

### 📦 配布物

- インストーラを v2.5.44 に更新し、latest.json と SHA256（checksums.sha256）を整合。

### 🧩 利用者向けの主な反映内容

- 外部アクセスモニターを追加し、接続状態や通信状況を把握しやすく改善。
- TTLキャッシュ導入により、一覧や参照の再取得負荷を抑え、繰り返し操作を安定化。
- サブグループ完全性チェックを追加し、不足項目の事前確認をしやすく改善。
- マスタデータの CSV / XLSX 出力に対応し、確認や共有を進めやすく改善。

### 📚 ドキュメント

- VERSION.txt / README / CHANGELOG / リリースノート等の表記を v2.5.44 に統一。
- docs/RELEASE_NOTES_v2.5.44.md を追加。

## v2.5.43 (2026-03-30) - 配布物整合 + データ取得2 / 一括取得（DP）応答性改善反映

### 📦 配布物

- インストーラを v2.5.43 に更新し、latest.json と SHA256（checksums.sha256）を整合。

### 🧩 利用者向けの主な反映内容

- データ取得2の初期表示を軽量化し、起動直後の待ち時間を抑制。
- 一括取得（DP）のフィルタ候補準備を非同期化し、タブ表示直後から操作しやすく改善。
- 一括取得（DP）の進捗表示を強化し、件数や処理状況を把握しやすく改善。
- モード切替時の再描画や再生成まわりの表示品質を改善。

### 📚 ドキュメント

- VERSION.txt / README / CHANGELOG / リリースノート等の表記を v2.5.43 に統一。
- docs/RELEASE_NOTES_v2.5.43.md を追加。

## v2.5.42 (2026-03-29) - 配布物整合 + データポータル操作導線改善反映

### 📦 配布物

- インストーラを v2.5.42 に更新し、`latest.json` と SHA256（`checksums.sha256`）を整合。

### 🧩 利用者向けの主な反映内容

- データポータルの主要操作ボタンを 1 行のアクション列へ整理し、書誌情報JSON・ZIP・修正・公開表示・ステータス変更を扱いやすく改善。
- 主要ボタンの見た目を compact 化し、hover / disabled を含む状態表現を分かりやすく整理。
- 共有データセット選択ドロップダウンの表示安定性を改善し、統合画面でも視認性を保つよう調整。

### 📚 ドキュメント

- VERSION.txt / README / CHANGELOG / リリースノート等の表記を v2.5.42 に統一。
- `docs/RELEASE_NOTES_v2.5.42.md` を追加。

## v2.5.41 (2026-03-28) - 配布物整合 + AI prompt assembly/辞書管理強化反映

### 📦 配布物

- インストーラを v2.5.41 に更新し、`latest.json` と SHA256（`checksums.sha256`）を整合。

### 🧩 利用者向けの主な反映内容

- データセット説明 AI提案、Quick AI、AI CHECK で使うテンプレートを個別に選べるよう改善。
- AI 実行前に prompt assembly の方式を切り替えられるようにし、運用に応じた使い分けをしやすく改善。
- filtered_embed 辞書の管理・要約確認・候補評価を強化し、AI 補助の調整を進めやすく改善。
- AI提案系のデータセット選択で入力絞り込みと表示安定性を改善。

### 📚 ドキュメント

- VERSION.txt / README / CHANGELOG / リリースノート等の表記を v2.5.41 に統一。
- `docs/RELEASE_NOTES_v2.5.41.md` を追加。

## v2.5.40 (2026-03-27) - 配布物整合 + AI設定/ローカルLLM強化反映

### 📦 配布物

- インストーラを v2.5.40 に更新し、`latest.json` と SHA256（`checksums.sha256`）を整合。

### 🧩 利用者向けの主な反映内容

- AI設定で生成パラメータを個別に調整できるよう改善。
- AI問い合わせの再試行回数を設定可能にし、失敗時の追跡性を向上。
- ローカルLLM設定を Ollama / LM Studio で分けて扱えるよう整理。
- AIテスト画面の初期化中表示を見直し、状態を把握しやすく改善。

### 📚 ドキュメント

- VERSION.txt / README / CHANGELOG / リリースノート等の表記を v2.5.40 に統一。
- `docs/RELEASE_NOTES_v2.5.40.md` を追加。

## v2.5.39 (2026-03-19) - 配布物整合 + 試料一覧2の再読込安定化反映

### 📦 配布物

- インストーラを v2.5.39 に更新し、`latest.json` と SHA256（`checksums.sha256`）を整合。

### 🧩 利用者向けの主な反映内容

- 試料一覧2の再読込経路を整理し、更新ボタンや再リンク後の表示安定性を改善。
- dataset detail JSON を使った補完を強化し、関連付け表示が欠けるケースを抑制。
- 課題番号解決で dataset 側の実データを優先し、一覧表示の整合性を改善。

### 📚 ドキュメント

- VERSION.txt / README / CHANGELOG / リリースノート等の表記を v2.5.39 に統一。
- `docs/RELEASE_NOTES_v2.5.39.md` を追加。

## v2.5.38 (2026-03-19) - 配布物整合 + 試料一覧2の表示改善反映

### 📦 配布物

- インストーラを v2.5.38 に更新し、`latest.json` と SHA256（`checksums.sha256`）を整合。

### 🧩 利用者向けの主な反映内容

- 試料一覧2のタイル数フィルタ初期値を調整し、初期表示時の扱いやすさを改善。
- 遅延読込中のセルに `読み込み中` を表示し、空欄との判別をしやすく改善。
- 参照エントリーが無い行を `エントリー無し` と明示し、通常エントリーとの見分けやすさを向上。

### 📚 ドキュメント

- VERSION.txt / README / CHANGELOG / リリースノート等の表記を v2.5.38 に統一。
- `docs/RELEASE_NOTES_v2.5.38.md` を追加。

## v2.5.37 (2026-03-18) - 配布物整合 + AI支援ダイアログ表示改善

### 📦 配布物

- インストーラを v2.5.37 に更新し、`latest.json` と SHA256（`checksums.sha256`）を整合。

### 🧩 利用者向けの主な反映内容

- AI支援ダイアログの初回表示サイズと位置を見直し、画面中央付近で扱いやすい表示へ改善。
- 「ファイル抽出設定」タブの初回表示サイズを調整し、表示直後に画面内へ収まりやすい挙動を反映。
- 回帰テストとビルド導線の更新済み内容を公開配布物へ反映。

### 📚 ドキュメント

- VERSION.txt / README / CHANGELOG / リリースノート等の表記を v2.5.37 に統一。
- `docs/RELEASE_NOTES_v2.5.37.md` を追加。

## v2.5.36 (2026-03-17) - 配布物整合 + 一括登録フォーム展開不整合修正

### 📦 配布物

- インストーラを v2.5.36 に更新し、`latest.json` と SHA256（`checksums.sha256`）を整合。

### 🧩 利用者向けの主な反映内容

- データ登録/タイルの一括登録タブで、データセット選択後に固有情報件数が存在していても「入力」切替でフォーム本体が展開されない不具合を改善。
- 固有情報フォーム本体とプレースホルダーの表示切替を整理し、summary 状態からでも操作が分かりやすくなるよう反映。
- pytest / pre-build / PyInstaller のリリース導線更新を公開配布物へ反映。

### 📚 ドキュメント

- VERSION.txt / README / CHANGELOG / リリースノート等の表記を v2.5.36 に統一。
- `docs/RELEASE_NOTES_v2.5.36.md` を追加。

## v2.5.35 (2026-03-16) - 配布物整合 + ファイルセット内容ダイアログの完了通知整理

### 📦 配布物

- インストーラを v2.5.35 に更新し、`latest.json` と SHA256（`checksums.sha256`）を整合。

### 🧩 利用者向けの主な反映内容

- データ登録/タイルの一括登録タブにある「ファイルセット内容」ダイアログで、適用後に操作不能へ見える不具合を改善。
- 内容ダイアログを閉じた後に完了通知を出すよう整理し、モーダル表示まわりの混乱を抑制。
- pre-build の段階ゲート安定化と、PyInstaller 生成 exe の VersionInfo 整合を反映。

### 📚 ドキュメント

- VERSION.txt / README / CHANGELOG / リリースノート等の表記を v2.5.35 に統一。
- `docs/RELEASE_NOTES_v2.5.35.md` を追加。

## v2.5.34 (2026-03-16) - 配布物整合 + 登録状況タブのレイアウト追従改善

### 📦 配布物

- インストーラを v2.5.34 に更新し、`latest.json` と SHA256（`checksums.sha256`）を整合。

### 🧩 利用者向けの主な反映内容

- データ登録/タイルの「登録状況」タブで、ウィンドウ高さを広げた際にテーブル領域が追従するよう改善。
- レイアウト配分を見直し、下側の不要な余白が残りにくい表示へ調整。
- 回帰テスト追加済みの反映内容を公開配布物へ取り込み。

### 📚 ドキュメント

- VERSION.txt / README / CHANGELOG / リリースノート等の表記を v2.5.34 に統一。
- `docs/RELEASE_NOTES_v2.5.34.md` を追加。


## v2.5.32 (2026-03-11) - 配布物整合 + UI安定化反映

## v2.5.33 (2026-03-12) - 配布物整合 + 読込表示/キャッシュ改善反映

### 📦 配布物

- インストーラを v2.5.33 に更新し、`latest.json` と SHA256（`checksums.sha256`）を整合。

### 🧩 利用者向けの主な反映内容

- タブ初期表示の遅延読み込みを反映し、起動直後の体感負荷を軽減。
- 基本情報タブとAIテスト2の読込中表示改善を反映。
- データポータル一覧の portal status 永続キャッシュ再利用と、全件強制更新経路の共通化を反映。

### 📚 ドキュメント

- VERSION.txt / README / CHANGELOG / リリースノート等の表記を v2.5.33 に統一。
- `docs/RELEASE_NOTES_v2.5.33.md` を追加。

## v2.5.32 (2026-03-11) - 配布物整合 + UI安定化反映

### 📦 配布物

- インストーラを v2.5.32 に更新し、`latest.json` と SHA256（`checksums.sha256`）を整合。

### 🧩 利用者向けの主な反映内容

- ウィンドウサイズ/テーマ管理の共通化により、画面ごとの表示安定性を改善。
- データポータル各タブのサイズ保持とテーマ再適用の安定化を反映。
- AIサジェストダイアログとサブグループ作成UIの状態保持改善を反映。

### 📚 ドキュメント

- VERSION.txt / README / CHANGELOG / リリースノート等の表記を v2.5.32 に統一。
- `docs/RELEASE_NOTES_v2.5.32.md` を追加。

## v2.5.0 (2026-01-19) - 配布物整合（latest.json/checksum/README）

### 📦 配布物

- インストーラ差し替えに合わせて、更新情報（`latest.json`）と改変チェック（`checksums.sha256`）を v2.5.0 に整合。

### 📚 ドキュメント

- VERSION.txt / README / CHANGELOG / リリースノート等の表記を v2.5.0 に統一。
- `docs/RELEASE_NOTES_v2.5.0.md` を追加。

## v2.4.18 (2026-01-13) - データセット一覧: 日時列の整合性修正 + 範囲フィルタ安定化
...
- 開設日(created) / 更新日(modified) / 公開日(openAt) の割当を仕様に合わせて統一。
...
- VERSION.txt / README / CHANGELOG / リリースノート等の表記を v2.4.18 に統一。
- `docs/RELEASE_NOTES_v2.4.18.md` を追加。

## v2.4.16 (2026-01-11) - テーマ切替: 破棄順序クラッシュ修正 + 一覧/各種UIの配色追従

## v2.4.17 (2026-01-12) - 配布物更新（インストーラ/sha256）+ 表記更新
...
- VERSION.txt / README / CHANGELOG / リリースノート等の表記を v2.4.17 に統一。
- `docs/RELEASE_NOTES_v2.4.17.md` を追加。
...
- VERSION.txt / README / CHANGELOG / リリースノート等の表記を v2.4.16 に統一。
- `docs/RELEASE_NOTES_v2.4.16.md` を追加。

---

## v2.4.15 (2026-01-10) - 更新ダウンロード: 通信ログ/バイト進捗の可視化 + スレッド安全化 + 正常終了メッセージ
...
- VERSION.txt / README / CHANGELOG / リリースノート等の表記を v2.4.15 に統一。
- `docs/RELEASE_NOTES_v2.4.15.md` を追加。

---

## v2.4.14 (2026-01-09) - データセット一覧: ポータル列の公開状態表示改善 + キャッシュ最適化

### 🧩 データセット一覧タブ: ポータル列（公開状態）

- ポータル列の公開状態表示を改善（公開JSON補完/ログインCSV一括判定/セルクリック再確認+セル内スピナー）。

### ⚡ パフォーマンス/ネットワーク負荷

- 個別アクセス回数を抑制（キャッシュの1日維持・上書き保存、クリック時のみ再確認）。
...
- VERSION.txt / README / CHANGELOG / リリースノート等の表記を v2.4.14 に統一。
- `docs/RELEASE_NOTES_v2.4.14.md` を追加。

---

## v2.4.13 (2026-01-09) - データセット一覧: 範囲フィルタ2行化 + TAG数範囲フィルタ修正 + file_size bytes統一
...
- VERSION.txt / README / CHANGELOG / リリースノート等の表記を v2.4.13 に統一。
- `docs/RELEASE_NOTES_v2.4.13.md` を追加。

---

## v2.4.12 (2026-01-09) - 一覧タブ: 大規模データ環境の初回表示改善 + 非同期リロード安定化

### 🧩 一覧タブ: 大規模データ対応

- サブグループ一覧/データセット一覧タブ: 大規模データで重くなりがちな処理を抑制し、初回表示が終わらないように見えるケースを低減。

### 🔧 安定性

- 非同期リロードのスレッド寿命管理を改善し、タブ切替等での `QThread: Destroyed while thread is still running` エラーを抑止。
...
## v2.4.11 (2026-01-08) - サブグループ一覧: 表示/操作性改善 + フィルタUI安定化 + 表記更新

### 🧩 サブグループ一覧タブ: 表示/操作性

- 「課題」「メンバー」「関連データセット」「関連試料」など、複数要素の表示を改行区切りに統一。
- 関連データセット/試料を、セル内の要素ごとに個別クリックで開けるよう改善。
- 長い名称は省略表示 + ツールチップで全体表示（エクスポートは生値）。
- 行高さが過大にならないよう調整（画面高の50%で上限）。
...
- VERSION.txt / README / CHANGELOG / リリースノート等の表記を v2.4.11 に統一。
- `docs/RELEASE_NOTES_v2.4.11.md` を追加。

---

## v2.4.10 (2026-01-07) - データセット一覧: フィルタUI/表示高速化 + エクスポートUX改善 + 表記更新
...
- VERSION.txt / README / CHANGELOG / リリースノート等の表記を v2.4.10 に統一。
- `docs/RELEASE_NOTES_v2.4.10.md` を追加。

---


## v2.4.7 (2026-01-06) - 設定/更新フロー改善（上書きインストール耐性・自動再起動）+ UI調整 + テスト安定化 + 表記更新
...
- VERSION.txt / README / CHANGELOG / リリースノート等の表記を v2.4.7 に統一。
- `docs/RELEASE_NOTES_v2.4.7.md` を追加。

---


## v2.4.6 (2026-01-06) - テスト運用改善（長時間スイート安定化）+ 表記更新
...
- VERSION.txt / README / CHANGELOG / リリースノート等の表記を v2.4.6 に統一。
- `docs/RELEASE_NOTES_v2.4.6.md` を追加。

---

## v2.4.5 (2026-01-05) - 安定性/互換性改善（テスト/起動決定性）+ 規約準拠（パス/テーマ）+ 表記更新
...
- VERSION.txt / README / CHANGELOG / リリースノート等の表記を v2.4.5 に統一。
- `docs/RELEASE_NOTES_v2.4.5.md` を追加。
