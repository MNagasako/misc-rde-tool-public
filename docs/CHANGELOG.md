...
## v2.4.9 (2026-01-07) - 品質改善（表記統一・ドキュメント整備・運用手順の整理）

### 📚 表記/ドキュメント

- VERSION.txt / README / CHANGELOG / リリースノート等の表記を v2.4.9 に統一。
- ドキュメント構成の整理（配布リポジトリの保守性改善）。
...
## v2.4.8 (2026-01-07) - AI説明文提案: 報告書/結果一覧の運用性改善 + テスト/表記更新

### 🤖 AI（報告書/結果一覧）

- 報告書: 横断技術領域（主/副）列の追加と事前フィルタ対応。
- 結果一覧（対象=報告書）: 年度/機関コード/横断/重要（主/副）の列表示と絞り込み UI を追加。
- converted.xlsx と結合して値を補完し、空欄を抑制（`target_key` が課題番号のみの場合など）。

### 🧪 テスト/品質

- 上記変更に追従してウィジェットテストを更新。

### 📚 ドキュメント

- VERSION.txt / README / CHANGELOG / リリースノート等の表記を v2.4.8 に統一。
- `docs/RELEASE_NOTES_v2.4.8.md` を追加。
