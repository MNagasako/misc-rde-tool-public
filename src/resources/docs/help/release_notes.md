...
| AI説明文提案 | AI拡張モードの初回サイズ/中央配置を見直し、表示直後に右へ寄る不整合を修正。 |
| ファイル抽出設定 | 初回高さをディスプレイ高さ基準にし、初回幅をメインウィンドウ初期幅へ揃えたうえで、後続の手動リサイズも維持。 |
| テスト/ビルド | geometry 回帰テストを追加更新し、pytest段階ゲート・pre-build・PyInstaller ビルド導線を v2.5.37 へ同期。 |
