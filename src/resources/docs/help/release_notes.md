...
| データセット（統合タブ） | タブごとにウィンドウサイズを独立保存/復元し、新規開設/新規開設2の初期幅をログイン幅基準へ調整。 |
| データセット（新規開設2） | コンボボックスの過大幅化を抑制し、初回表示時の横スクロール暴走を軽減。 |
| データセット（閲覧・修正/データエントリー） | 「管理（自身が開設・所有）」でもサブグループフィルタを利用可能化。 |
| データセット（閲覧・修正/データエントリー） | managed時の候補を「自身が所属するTEAMサブグループ」に限定。 |
| テスト/品質 | managed時サブグループ有効化/候補制限の回帰テストを更新。 |
