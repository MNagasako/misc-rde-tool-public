...
| データセット（新規開設） | 上位フィルタ（ロール/サブグループ）変更時に、下位フィルタ（課題番号/管理者）を安全側で初期化して候補を即時更新。 |
| データセット（新規開設2） | 新規開設と同一の共通ロジックで、サブグループ変更時に課題番号を先頭候補へ自動選択。 |
| 応答性/安定性 | 不整合な前回選択の保持を抑止し、コンボフォーカス時・一覧表示時の遅延や無応答に見える挙動を改善。 |
| テスト/品質 | `test_dataset_open_filter_cascade_resets_to_first_defaults.py` を追加し、上位変更時の下位初期化を回帰検証。 |
