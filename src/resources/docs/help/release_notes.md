...
| データ登録（通常/一括） | テンプレート対応形式表示を、テンプレート名完全一致依存から装置ID・版・日付による解決方式へ拡張。 |
| データ登録（通常/一括） | 指定テンプレートが不一致の場合、旧版テンプレートへ段階フォールバックして対応拡張子判定を継続。 |
| データ登録（通常/一括） | 対応ファイル形式欄に「参照テンプレート名」「最新/旧版」「フォールバック有無」を表示。 |
| テスト/品質 | `test_template_format_validator` を拡張し、版フォールバックと参照表示の回帰を追加。 |
