...
| サブグループ（閲覧・修正） | 部分一致候補（Completer）をクリックした直後に、コンボボックスの選択インデックスを即時同期するよう修正。 |
| サブグループ（閲覧・修正） | 選択変更通知を `currentIndexChanged` / `activated` でも受けるよう改善し、入力テキスト不変時の更新漏れを解消。 |
| テスト/品質 | 閲覧・修正タブの候補クリック選択に対する回帰テストを追加（クリック直後に `currentData` 更新を検証）。 |
