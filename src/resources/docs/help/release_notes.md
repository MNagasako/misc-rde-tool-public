...
| サブグループ（新規作成） | 表示フィルタ適用時に、表示されているメンバーのみを作成対象・選択表示対象として扱うよう修正。 |
| サブグループ（新規作成） | 同一メールの重複表示をマージし、手動追加エントリを優先して1行表示に統合。 |
| サブグループ（新規作成） | 手動追加時の重複判定を表示中行基準に変更し、非表示プリセット由来の誤ブロックを解消。 |
| サブグループ（新規作成） | 作成前バリデーションを一括化し、未達条件（グループ名・メンバー・ロール・OWNER）をまとめて表示。 |
