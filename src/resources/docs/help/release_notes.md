...
| データセット（新規開設/新規開設2） | ロールフィルタ変更時、選択中サブグループが新条件に不一致なら自動解除。 |
| データセット（新規開設/新規開設2） | サブグループ変更時、課題番号・データセット管理者の前回選択が新候補に無い場合は自動解除。 |
| データセット（新規開設/新規開設2） | フィルタ再構築時に無効な選択値保持を避け、応答遅延/選択不能化の再発を抑制。 |
| テスト/品質 | 上位フィルタ変更時の下位選択解除を検証するウィジェットテストを追加。 |
