...
| データセット（新規開設2） | 既存データセット読込の表示対象を「管理/所属機関の課題/その他/全て」に整理。 |
| データセット（閲覧・修正/データエントリー） | 表示対象フィルタを新規開設2と同じ4区分に統一。 |
| データセット（閲覧・修正/データエントリー） | サブグループフィルタは「所属機関の課題」「全て」のときのみ有効化。 |
| データセット（閲覧・修正/データエントリー） | dataset.jsonにgroup関係が無い場合でも、datasets/<id>.jsonのgroup関係を参照して絞り込み継続。 |
| テスト/品質 | フィルタ分類整合・サブグループ有効条件・詳細JSONフォールバックのウィジェット回帰を追加/更新。 |
