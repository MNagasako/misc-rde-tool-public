...
> **対象バージョン**: v2.2.3 / 最終更新日 2025-12-09
...
3. v2.2.3 では事前に表示される「プロジェクトファイル選択」ダイアログで以下を切り替え可能です。
   - 既定の groupOrganizations / groupProjects JSON を一括使用する従来方式
   - JSON ごとに個別ファイルへ分割し、必要な項目だけをまとめるパターン
   - 外部JSONや `attributes.name` フィルタを混在させたカスタム構成
4. 複数ファイルを出力した場合は結果一覧ダイアログが自動表示され、任意のファイルを直接開けます。`まとめフォルダを開く` ボタンで Explorer を起動して出力フォルダへ移動できます。
