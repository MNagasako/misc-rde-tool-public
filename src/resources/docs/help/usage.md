...
> **対象バージョン**: （アプリのタイトルに表示 / `VERSION.txt` 先頭行の値） / 最終更新日 2026-02-10
...
5. 複数ファイルを出力した場合は結果一覧ダイアログが自動表示され、任意のファイルを直接開けます。`まとめフォルダを開く` ボタンでファイルマネージャ（Windows: Explorer / macOS: Finder / Linux: Files など）を起動して出力フォルダへ移動できます。
...
## 9. データセット開設（新規開設/新規開設2）

- 「ロールフィルタ」で表示対象を絞り込めます（例: 管理者/管理者代理）。
- 「フィルタなし」を選ぶと絞り込みを解除します。
- 「サブグループフィルタ」「テンプレートフィルタ形式」「組織フィルタ」は補助的な絞り込み項目です。
- データセット修正タブのデータセット選択は、入力中でも ↑↓ キーやマウスホイールで候補選択できます（候補リストが開きます）。