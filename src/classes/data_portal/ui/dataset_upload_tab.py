...
class DatasetUploadTab(QWidget):
    ...
    def __init__(self, parent=None):
        ...
        self.current_grant_number = None
    ...
    def _on_dataset_selected_advanced(self, index: int):
        ...
        try:
            ...
            if json_path:
                ...
                info_text = self._build_dataset_info_text(
                    dataset_name,
                    dataset_id,
                    json_path,
                    files_exist,
    ...
    def _build_dataset_info_text(
        ...
        lines = []
        if dataset_name:
            lines.append(f"データセット: {dataset_name}")
        lines.append(f"ID: {dataset_id}")
        lines.append(f"JSONファイル: {Path(json_path).name}")

        try:
            json_stat = Path(json_path).stat()
            lines.append(f"取得日時: {self._format_file_timestamp(json_stat.st_ctime)}")
            lines.append(f"更新日時: {self._format_file_timestamp(json_stat.st_mtime)}")
        except OSError:
            lines.append("取得日時: 取得不可")
            lines.append("更新日時: 取得不可")

        image_text = '取得済み' if files_exist else '未取得'
        if files_exist:
            image_text += f" ({file_count}件)"
        lines.append(f"画像ファイル: {image_text}")

        return "\n".join(lines)

    def _refresh_dataset_json_for_upload(self, dataset_id: str) -> Optional[str]:
        try:
            from core.bearer_token_manager import BearerTokenManager
            ...
            bearer_token = BearerTokenManager.get_token_with_relogin_prompt(self)
            if not bearer_token:
                self._show_error("RDEのBearerトークンが取得できませんでした")
                return None

            json_path = Path(get_dynamic_file_path(f"output/rde/data/datasets/{dataset_id}.json"))
            before_bytes = json_path.read_bytes() if json_path.exists() else None

            self._log_status(f"RDEからデータセットJSONを再取得中: {dataset_id}")
            fetch_dataset_info_respectively_from_api(bearer_token, dataset_id)

            if not json_path.exists():
                raise FileNotFoundError(f"再取得後のJSONファイルが見つかりません: {json_path}")

            after_bytes = json_path.read_bytes()
            if before_bytes is not None and before_bytes != after_bytes:
                self._show_warning("ローカル保存のJSONが古かったため、RDEから最新版を再取得して置き換えました。")

            self.selected_json_path = str(json_path)
            self._log_status(f"✅ 最新のデータセットJSONを使用: {json_path.name}")
            return str(json_path)

        except Exception as e:
            logger.error(f"データセットJSON再取得エラー: {e}", exc_info=True)
            self._show_error(f"データセットJSON再取得エラー\n{e}")
            return None
    ...
    def _on_upload(self):
        ...
        if self.current_dataset_id:
            refreshed_json_path = self._refresh_dataset_json_for_upload(self.current_dataset_id)
            if not refreshed_json_path:
                return
            upload_json_path = refreshed_json_path
        if self.anonymize_checkbox.isChecked():
            self._log_status("匿名化処理中（課題番号をJSONから取得）...")
            upload_json_path = self._anonymize_json(upload_json_path)
