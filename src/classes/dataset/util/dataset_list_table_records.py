...
def clear_dataset_list_cache() -> None:
    ...
    This is mainly for tests and for explicit refresh flows.
    """

    _DATASET_LIST_CACHE["signature"] = None
    _DATASET_LIST_CACHE["columns"] = None
    _DATASET_LIST_CACHE["rows"] = None
...
def build_dataset_list_rows_from_files() -> Tuple[List[DatasetListColumn], List[Dict[str, Any]]]:
    ...
    signature = (
        ...
        _safe_mtime(template_json_path),
        _safe_mtime(instruments_json_path),
        _safe_mtime(licenses_json_path),
        _dir_signature(subgroups_dir),
        _dir_signature(data_entry_dir),
        _dir_signature(dataset_details_dir),
    )

    cached_sig = _DATASET_LIST_CACHE.get("signature")
    if cached_sig == signature and isinstance(_DATASET_LIST_CACHE.get("columns"), list) and isinstance(
        _DATASET_LIST_CACHE.get("rows"), list
    ):
