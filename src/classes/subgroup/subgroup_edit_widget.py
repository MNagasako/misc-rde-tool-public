...
class EditMemberManager:
    ...
    def update_member_selection(self, group_data, user_entries):
        ...
        # 新しいメンバーセレクターを作成
        new_member_selector = create_common_subgroup_member_selector(
...
class SubgroupSelector:
    ...
    def _update_combo_items(self):
        ...
        for group in sorted_groups:
            display_text = f"{group['name']} (ID: {group['id']})"
...
def create_subgroup_edit_widget(parent, title, color, create_auto_resize_button):
    ...
    managers = _initialize_managers(
        ...
        form_section['builder'], form_widgets
    )
...
def _initialize_managers(combo, filter_combo, scroll_area, form_builder, form_widgets):
    """管理クラスの初期化"""
    ...
    member_manager = EditMemberManager(scroll_area)
