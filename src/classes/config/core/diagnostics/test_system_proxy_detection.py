...
class SystemProxyDetector:
    ...
    def detect_windows_registry(self) -> Dict[str, object]:
        ...
        try:
            from classes.core.platform import get_windows_registry_proxy

            registry_info = get_windows_registry_proxy()
            proxy_enable = registry_info.get('ProxyEnable')
            proxy_server = registry_info.get('ProxyServer')
