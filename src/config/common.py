...
# 2. 配布物同梱テキスト: readme.txt, LICENSE-dist.txt
# NOTE: アプリ内のバージョン表示は VERSION.txt を単一SoTとし、REVISION はそこから読み取った値を公開する。

# 2026-01-14: v2.4.19 - リリース準備: バージョン表記統一 + pre-build統合テスト/ビルド
# 2026-01-13: v2.4.18 - データセット一覧: 開設日/更新日/公開日列の整合性修正 + 範囲フィルタ安定化
# 2026-01-11: v2.4.17 - テーマ切替: 破棄順序クラッシュ修正 + 一覧/各種UIの配色追従 + テスト更新
# 2026-01-10: v2.4.15 - 更新ダウンロード: 通信ログ/バイト進捗の可視化 + スレッド安全化 + 正常終了（更新のため終了）メッセージ
# 2026-01-09: v2.4.13 - データセット一覧: 範囲フィルタ2行化 + TAG数範囲フィルタ修正 + file_size bytes統一 + テスト/ドキュメント更新
# 2026-01-09: v2.4.14 - データセット一覧: ポータル列の公開状態表示改善（クリック再確認+セル内スピナー/ログインCSV一括判定/キャッシュ最適化）+ テスト/ドキュメント更新
# 2026-01-09: v2.4.12 - 一覧タブ: 無限スピナー対策（重い計算抑制）+ QThread寿命管理改善 + テスト/ドキュメント更新
# 2026-01-08: v2.4.11 - サブグループ一覧: 表示/リンク/行高さ/フィルタUI改善 + テスト
# 2026-01-07: v2.4.10 - データセット一覧タブ: フィルタUI改善 + キャッシュ高速化 + エクスポート既定名 + テスト/ビルド
# 2026-01-07: v2.4.9 - リビジョンアップ: バージョン表記統一・ドキュメント更新・テスト/ビルド
# 2026-01-07: v2.4.8 - AI説明文提案: 報告書/結果一覧の表示・絞り込み改善 + テスト/ドキュメント更新
