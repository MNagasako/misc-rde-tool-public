...
**ARIM事業 RDE→ARIMデータポータル　支援ツール v2.4.1** のバイナリ配布およびドキュメント公開用リポジトリです。
...
**2026-01-02 最新リリース**: バージョン2.4.1（データ取得2: メール通知タブ改善）

### v2.4.1 の主な改善

- 抽出条件: 範囲（日）を初期値7日・最大999日に変更し、プリセット（1/3/7/30/180/365日）で素早く切替可能にしました。
- 運用モード: 「テスト/本番」をラジオで選択し、誤送信防止と運用意図を明確化しました。
- 本番送信先: 「投入者/所有者/両方」を選択可能にし、通知先の運用要件に対応しました。
- 送信ログ: 送信ログの閲覧機能を追加し、二重送信防止（送信先+件名キー）と監査を支援します。
- From表示名: 設定タブ（Gmail/SMTP）に任意のFrom表示名を追加し、ヘッダへ反映しました。
...
* **[リリースノート v2.4.1](docs/RELEASE_NOTES_v2.4.1.md)**

---

## 🔢 バージョン情報

現在配布バージョン: `v2.4.1` （`VERSION.txt` に基づく）
