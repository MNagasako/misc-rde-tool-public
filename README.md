...
### 最新版主な機能（v1.15.0）
- ワークスペース大規模整理・不要ファイル削除・アーキテクチャ最適化
- コードベース品質向上（重複コード削除・依存関係整理・モジュール構造改善）
- 開発環境安定化（PyInstaller配布パッケージ最適化・ビルドプロセス改善）
- 設計書・仕様書・運用手順書の整備充実
- 継続的品質改善（静的解析・テストカバレッジ向上・CI/CD基盤強化）
- サブグループUI・データセット選択・AI分析・XLSX出力・リクエスト解析等の主要機能を安定提供

### リリース管理
- **GitHub Releases**: バイナリファイルのバージョン管理
- **タグ管理**: セマンティックバージョニング（v1.15.0形式）
