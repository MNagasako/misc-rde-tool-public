...
**ARIM事業 RDE→ARIMデータポータル支援ツール v2.5.11** のバイナリ配布およびドキュメント公開用リポジトリです。  
...
**2026-02-14 最新リリース**: バージョン2.5.11（配布物整合）

### v2.5.11 の主な変更点

- **配布整合**: 更新判定（`latest.json`）と改変チェック（`checksums.sha256`）を v2.5.11 に整合。
- **導線更新**: README の最新版表記・ダウンロード導線を v2.5.11 に更新。
...
> ✅ 現在のファイル名は `arim_rde_tool_setup.2.5.11.exe` です（改変チェックは `latest.json` の `sha256` または `checksums.sha256` を参照）。
#### 最新版バイナリへの直接リンク
...
現在配布バージョン: `v2.5.11` （`VERSION.txt` に基づく）
