...
Windows向け配布版 v2.5.60 のインストーラと利用者向けドキュメントを公開しています。
...
v2.5.60 では、データポータル編集画面の保存確認ダイアログにペイロードプレビューを追加し、編集内容の確認と検証をより詳細に行えるようにしました。また、チェックボックス系配列フィールドのクリア挙動を強化し、データセットアップロード時のキャッシュ作成日時表示を改善しています。

## 2) インストール方法

1. [Releases](https://github.com/MNagasako/misc-rde-tool-public/releases) から最新版インストーラを取得
2. arim_rde_tool_setup.2.5.60.exe を実行
3. 画面の案内に従ってインストール
...
- リリースノート: [docs/RELEASE_NOTES_v2.5.60.md](docs/RELEASE_NOTES_v2.5.60.md)
