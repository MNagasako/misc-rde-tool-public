...
**ARIM事業 RDE→ARIMデータポータル支援ツール v2.5.24** のバイナリ配布およびドキュメント公開用リポジトリです。
...
**2026-02-27 最新リリース**: バージョン2.5.24

### v2.5.24 の主な変更点

- **更新配布**: `arim_rde_tool_setup.2.5.24.exe` を公開し、`latest.json` / `checksums.sha256` を同版に整合。
- **機能改善**: データポータル運用機能の安定性と日常運用時の使い勝手を継続改善。
- **利用者向け更新**: インストール・更新導線とバージョン表記を最新版へ統一。

## ✅ 最短利用ガイド

1. **できること**: RDE操作補助、データポータル連携、AI支援を1つのWindowsアプリで実行できます。
2. **インストール**: Releases から `arim_rde_tool_setup.2.5.24.exe` を実行するだけで導入できます。
...
> ✅ 現在のファイル名は `arim_rde_tool_setup.2.5.24.exe` です（改変チェックは `latest.json` の `sha256` または `checksums.sha256` を参照）。
#### 最新版バイナリへの直接リンク
