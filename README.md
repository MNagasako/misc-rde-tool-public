...
1　グループに追加するメンバーの役割を設定してください。
 - メンバーリストにはログイン中のユーザーが所属するサブグループのメンバーが含まれています。
 - `input/rde-member.txt` でメンバーの役割のデフォルト値を指定できます。
 - リストされているメンバー以外を追加する場合は、メールアドレスを入力しユーザー追加をクリックします。（RDEに登録されているユーザーの場合は、リストに追加されます。）
2 グループ名、説明、課題番号、研究資金番号　を設定します。
3 作成ボタンをクリック　（一括作成ボタンは現在つかえません。アプリがクラッシュします）

修正
1　サブグループ選択ドロップダウンリストで修正対象のサブグループ（研究グループ）を選択します。
　補完検索可
　表示フィルタでリスト内エントリーの絞り込みが可能です。
2　サブグループの内容を変更する場合は、サブグループ更新をクリックします。
・RDEサブグループページを開くボタン　ー＞選択中のサブグループのページをRDE上で開きます。　（事前にブラウザ側でもログインを済ませておく必要があります。）
・関連試料抽出ボタン　->選択中のサブグループに属する試料一覧を表示　－＞クリックで、RDEのサンプルページを開きます。（試料情報の編集や削除はRDEサイト上が便利です。）
