...
現時点で、RDEサイト上で実現可能な機能の大半を実装しております。


- ログイン情報の保存（DICEアカウントのみ対応、NIMSアカウントは未調査）
  - ログイン情報は「暗号化ファイル」または「OSキーチェーン」に安全に保存可能です（推奨）。
  - 旧来の`login.txt`は非推奨です。設定メニューから安全な保存先へ移行してください。
- プロキシ環境対応（CA証明書・PACファイル対応。ブラウザでアクセスできていれば基本的に利用可能です）
  - 認証プロキシ（ユーザー名・パスワードが必要なプロキシ）は非対応です。
- RDEサイトへのリンク（アプリから新規タブでRDEページを開くことが可能です）
...
## 開発中
- データ登録AI連携（データ名・説明・実験ID・タグのサジェスト）
- 一括登録（フォルダ指定→ファイルツリー取得→ファイルセット単位で登録・パスマッピング出力）
...
## 注意事項・免責
- 本ツールは**個人開発のため無保証**です。
- ドキュメントおよびコードにはAIによる自動生成部分が含まれております。
- 動作検証は実施しておりますが、説明文や文言等の細部まで厳密な確認を行っていない場合がございます。
  内容の正確性・完全性については保証いたしかねますので、ご利用にあたっては利用者ご自身のご判断でお願いいたします。
...
本プロジェクトのソースコードは **GNU General Public License v3.0**（**GPL-3.0**）の下で提供しています。
- 本プロジェクトは **PyQt5 を GPLv3 条件で利用**しており、**Riverbank の商用ライセンスは使用していません**。

- [LICENSE（本体ライセンス）](./LICENSE)
- [THIRD_PARTY_NOTICES（サードパーティライセンス通知）](./THIRD_PARTY_NOTICES)
