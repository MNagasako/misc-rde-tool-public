...
Windows向け配布版 v2.5.61 のインストーラと利用者向けドキュメントを公開しています。
...
v2.5.61 では、データポータル書誌情報 JSON の匿名化処理を改善し、`included` 内の `type == user` 要素を確実に除去できるようにしました。あわせて、user 要素除去のみが差分だった場合でも匿名化済みファイルを安定して出力できるようにし、アップロード前確認の見通しを改善しています。

## 2) インストール方法

1. [Releases](https://github.com/MNagasako/misc-rde-tool-public/releases) から最新版インストーラを取得
2. arim_rde_tool_setup.2.5.61.exe を実行
3. 画面の案内に従ってインストール
...
- リリースノート: [docs/RELEASE_NOTES_v2.5.61.md](docs/RELEASE_NOTES_v2.5.61.md)
