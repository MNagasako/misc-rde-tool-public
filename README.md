...
**ARIM事業 RDE→ARIMデータポータル　支援ツール v2.4.0** のバイナリ配布およびドキュメント公開用リポジトリです。
...
**2026-01-01 最新リリース**: バージョン2.4.0（データ登録: 登録状況タブ拡張）

### v2.4.0 の主な改善

- データ登録: 「登録状況」タブに自動確認（10秒ポーリング）を追加し、進捗の可視化と手動停止トグルに対応しました。
- 表示改善: リンク列縮小・水平スクロール対応により、見切れ/操作性を改善しました。
- 情報追加: 投入者/所有者(所属)/エラーコード/エラーメッセージ列を追加し、確認性を改善しました。
- 高速化: entry detail の個別JSON保存と、キャッシュがある場合の高速表示/部分更新に対応しました。
...
現在配布バージョン: `v2.4.0` （`VERSION.txt` に基づく）
