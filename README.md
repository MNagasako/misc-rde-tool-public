...
**ARIM事業 RDE→ARIMデータポータル　支援ツール v2.3.4** のバイナリ配布およびドキュメント公開用リポジトリです。
...
**2025-12-23 最新リリース**: バージョン2.3.4（データポータル修正 - 関連カタログ生成 + テスト安定化）

### v2.3.4 の主な改善

- データポータル修正（装置・プロセス）: 装置・プロセス領域に機能列（関連カタログ）を追加し、関連カタログHTML生成をサポートしました。
- データポータル修正（装置・プロセス）: 見出しテキスト/見出しタグ（既定h2）を確認ダイアログから編集可能にしました。
- テスト: xfail/xpass整理・ダイアログ/スレッドのスタブ化により、テストのフレークを抑制しました。
- README / VERSION.txt / CHANGELOG / リリースノート等のバージョン表記を v2.3.4 に統一しました。
