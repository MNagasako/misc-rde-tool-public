...
**ARIM事業 RDE→ARIMデータポータル移行ツール v2.1.24** のバイナリ配布およびドキュメント公開用リポジトリです。
...
**2025-12-06 最新リリース**: バージョン2.1.24（サブグループ完全性チェック・UI安定化）

### v2.1.24 の主な改善

- サブグループ完全性チェックを強化し、欠損検知時は必ず再取得ループを実行
- 取得ループの詳細ログ（成功/失敗/スキップ件数・force_download有無）を出力
- グループ選択ダイアログをUIスレッド同期実行に固定し、基本情報取得時のクラッシュを解消
- データセットフィルタの既定プログラムを `all` に統一して初期表示を安定化
- 認証情報保存を `encrypted_data` キーでラップし、既存データとも後方互換
