...
**ARIM事業 RDE→ARIMデータポータル　支援ツール v2.3.13** のバイナリ配布およびドキュメント公開用リポジトリです。
...
**2025-12-29 最新リリース**: バージョン2.3.13（データ構造化XLSX: V6/V7混在のテンプレ列を行ごとに最新版選択 + テスト追加）

### v2.3.13 の主な改善

- 設定→データ構造化（XLSX）で、テンプレートID/名の取得を改善しました。
	- 従来: 「最新列固定」（例: V7列のみ参照）
	- 今回: 行単位で「値が入っている最大Vn」の列を採用（V6/V7混在に対応）
- 上記挙動をユニットテストで固定し、回帰を防止しました。
...
現在配布バージョン: `v2.3.13` （`VERSION.txt` に基づく）
