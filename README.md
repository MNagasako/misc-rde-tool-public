...
| バージョン | 日付 | 概要 |
| :-----: | :--------: | :------------------------ |
| v2.1.5 | 2025-11-17 | **マイナーアップデート**: プロジェクト構成整理・ネットワークアーキテクチャ詳細ドキュメント化・品質改善継続 |
...
* **v2.1.5新機能**: プロジェクト構成整理・ネットワークアーキテクチャ詳細ドキュメント化・品質改善継続
...
* データセット説明欄AIサジェスト機能（説明文の自動生成・補助）
* RDE情報の一括取得・XLSX出力
* 柔軟なフィルタ・検索・補完機能
...
> **注意（v2.1.4）**: PySide6への移行により、一部WebViewやUI動作に細かな差異が生じる可能性があります。旧版で問題なく動作していた環境でも、念のため動作確認を推奨します。
...
- **v2.1.4での変更**: PySide6への移行により、PyQt5版と比較して一部のWebView動作やUI描画に細かな差異が生じる可能性があります。動作確認済みですが、特定環境で想定外の挙動が生じた場合はご報告ください
...
- **v2.1.4完了**: PySide6への完全移行完了・LGPLv3ライセンス採用
- 一括登録・サブグループ編集・AIチャット・安定性向上
