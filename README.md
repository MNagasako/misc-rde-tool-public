...
**ARIM事業 RDE→ARIMデータポータル支援ツール v2.5.25** のバイナリ配布およびドキュメント公開用リポジトリです。
...
**2026-03-01 最新リリース**: バージョン2.5.25

### v2.5.25 の主な変更点

- **更新配布**: `arim_rde_tool_setup.2.5.25.exe` を公開し、`latest.json` / `checksums.sha256` を同版に整合。
...
2. **インストール**: Releases から `arim_rde_tool_setup.2.5.25.exe` を実行するだけで導入できます。
...
> ✅ 現在のファイル名は `arim_rde_tool_setup.2.5.25.exe` です（改変チェックは `latest.json` の `sha256` または `checksums.sha256` を参照）。
#### 最新版バイナリへの直接リンク
