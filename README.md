...
Windows向け配布版 v2.5.62 のインストーラと利用者向けドキュメントを公開しています。
...
v2.5.62 では、報告書タブの一括処理で Excel 出力時に `IllegalCharacterError` が起きるケースを改善しました。Excelに書き込めない制御文字を出力前に除去し、処理が途中で止まりにくいようにしています。

## 2) インストール方法

1. [Releases](https://github.com/MNagasako/misc-rde-tool-public/releases) から最新版インストーラを取得
2. arim_rde_tool_setup.2.5.62.exe を実行
3. 画面の案内に従ってインストール
