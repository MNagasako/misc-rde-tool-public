...
Windows向け配布版 v2.5.59 のインストーラと利用者向けドキュメントを公開しています。
...
v2.5.59 では、データ取得APIのページ上限を引き上げ、多件数データセットでの取得効率を改善し、キャッシュ参照時のパス管理を修正して実行環境差異による不整合を減らしました。

## 2) インストール方法

1. [Releases](https://github.com/MNagasako/misc-rde-tool-public/releases) から最新版インストーラを取得
2. arim_rde_tool_setup.2.5.59.exe を実行
3. 画面の案内に従ってインストール
...
- リリースノート: [docs/RELEASE_NOTES_v2.5.59.md](docs/RELEASE_NOTES_v2.5.59.md)
