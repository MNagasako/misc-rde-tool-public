...
**ARIM事業 RDE→ARIMデータポータル移行ツール** のバイナリ配布およびドキュメント公開用リポジトリです。

本ツールは、文部科学省ARIM事業の研究データ共有サイト「RDE」の操作を補助するとともに、データポータルへの登録を支援するWindowsデスクトップアプリケーションです。
Web操作の煩雑さを解消し、データの一括取得や登録、AIによる支援機能を提供します。
...
問題報告・改善提案は GitHub Issues を利用してください:
https://github.com/MNagasako/misc-rde-tool-public/issues
...
* 一部データは高速化のためローカルキャッシュされます。削除やクリア方法は今後追加予定のオプションで提供。
* 企業プロキシ環境対応機能あり。通信設定は組織ポリシーに従って構成してください。
...
v2.0.0 以降、ライセンス変更（LGPLv3）とともに「今後はソースコードを恒常的には公開しない」方針に移行しました。既存公開分は移行期間中のみ参照可能な暫定措置です。

---

## ✅ チェックリスト（初回利用）

1. 最新版を Releases から取得
2. 起動後 `input/` / `output/` が生成されたことを確認
3. ログイン設定を保存（必要に応じてテスト接続）
4. 機能別ドキュメントで目的機能を確認
5. 出力結果を `output/` で検証
...
* RDE 側仕様変更に伴い一部機能が停止する可能性
* WebView 実装差異（PySide6 移行後）により表示差分が出る場合
* キャッシュ削除オプションは今後追加予定
...
* ネットワーク設定後、接続テスト機能で疎通確認
* データ取得機能で最小件数（1件）を試験取得
* AI 機能はサンプルプロンプトで応答確認（API キー設定が必要な場合あり）

---

## 📁 関連ディレクトリ（概要列挙）

（内容詳細は列挙のみ。自明でない内部仕様は記載しません）
