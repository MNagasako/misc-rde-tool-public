...
**ARIM事業 RDE→ARIMデータポータル　支援ツール v2.3.14** のバイナリ配布およびドキュメント公開用リポジトリです。
...
**2025-12-30 最新リリース**: バージョン2.3.14（UI黒化対策 + RDE欠落ID除外 + テスト安定化/規約準拠）

### v2.3.14 の主な改善

- UI: 低頻度で発生する「黒い未塗り領域」を抑止するため、スクロール領域の背景塗りを改善しました。
- データ整合性: RDE 側で削除済み（404/410確定）のデータセット/サブグループIDを候補から除外し、ローカルJSONとの乖離によるエラーを回避します。
- テスト/品質: pytest 実行時の固まりを抑制し、規約準拠を整理しました。
...
現在配布バージョン: `v2.3.14` （`VERSION.txt` に基づく）
