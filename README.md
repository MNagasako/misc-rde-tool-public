...
**ARIM事業 RDE→ARIMデータポータル　支援ツール v2.3.16** のバイナリ配布およびドキュメント公開用リポジトリです。
...
**2025-12-31 最新リリース**: バージョン2.3.16（データエントリー編集（送り状）改善 + 編集ダイアログUI改善）

### v2.3.16 の主な改善

- データエントリー編集: 一覧から送り状（invoice）の編集ダイアログを開き、更新できるように改善しました。
- 基本情報: データ所有者（所属）を選択して更新できるようにしました。
- 試料情報: 「前回と同じ / 既存試料ID / 新規作成」の3択モードを維持して更新します。
- UI/UX: 編集ダイアログをスクロール対応にし、初期サイズやレイアウト（2列）を最適化しました。
...
現在配布バージョン: `v2.3.16` （`VERSION.txt` に基づく）
