...
**ARIM事業 RDE→ARIMデータポータル　支援ツール v2.4.11** のバイナリ配布およびドキュメント公開用リポジトリです。  
...
**2026-01-08 最新リリース**: バージョン2.4.11（サブグループ一覧: 表示/操作性改善 + フィルタUI安定化 + 表記更新）

### v2.4.11 の主な改善

- サブグループ一覧タブ: 「課題」「メンバー」「関連データセット」「関連試料」など複数要素の表示を改行区切りに統一。
- サブグループ一覧タブ: 関連データセット/試料をセル内の要素ごとに個別クリックで開けるよう改善。
- サブグループ一覧タブ: 長い名称は省略表示 + ツールチップで全体表示（エクスポートは生値）。
- フィルタUI: 範囲フィルタの折り返し時にラベルが重なって見える問題を修正。
- 配布版ドキュメントの表記（VERSION/README/CHANGELOG/リリースノート）を v2.4.11 に統一。
