...
**ARIM事業 RDE→ARIMデータポータル　支援ツール v2.4.4** のバイナリ配布およびドキュメント公開用リポジトリです。

**本ページはAIによる自動生成文が含まれています。**

**[ARIM-RDE-TOOL　置き場](https://cuddly-stinger-40d.notion.site/QuickStart-2befc2cb5fc380f09d0dd4595b767f4d?source=copy_link)**　（外部サイト）(こちらのほうが人力なので間違いが少ないです。更新頻度も高いです)
...
**2026-01-04 最新リリース**: バージョン2.4.4（メール通知タブ: 自動通知の送信対象にリストフィルタ適用 + 基準日時モード + テンプレ運用改善）

### v2.4.4 の主な改善

- 対象通知リストのフィルタ（エラーコード/設備ID/管理者名/管理者メール）を、自動通知の送信対象にも適用しました（表示されていないものは送信しません）。
- 「通知対象抽出」で、抽出前に最新情報を取得するか選択できるオプションを追加しました（自動通知と同等の挙動）。
- 基準日時の指定方式を「自動（常に現在）/直接指定」で切替可能にし、既定は自動（抽出実行時点の現在）にしました。
- 削除済みテンプレートが再起動で復活する不具合を抑止しました（移行済みマーカー導入）。
...
* **[リリースノート v2.4.4](docs/RELEASE_NOTES_v2.4.4.md)**

---

## 🔢 バージョン情報

現在配布バージョン: `v2.4.4` （`VERSION.txt` に基づく）
