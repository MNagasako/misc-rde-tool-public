...
**ARIM事業 RDE→ARIMデータポータル　支援ツール v2.3.15** のバイナリ配布およびドキュメント公開用リポジトリです。
...
**2025-12-31 最新リリース**: バージョン2.3.15（AI説明文提案: データセットタブ追加 + ログビューア改善 + テスト安定化）

### v2.3.15 の主な改善

- AI説明文提案: 「データセット」タブを追加し、一覧テーブル選択 + 列別フィルタ方式に対応しました。
- ログビューア: 結果一覧から開くログ表示で、JSONをツリー表示/全文表示で切り替え可能にし、標準テキストエディタで開く操作を追加しました。
- テスト/安定性: pytest実行時のテーマ切替負荷を抑制し、長時間スイートでのタイムアウト発生を低減しました。
...
現在配布バージョン: `v2.3.15` （`VERSION.txt` に基づく）
