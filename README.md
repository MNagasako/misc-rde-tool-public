...
> ドキュメントの多くはAIによる自動生成が含まれているため、誤りが混在する可能性があります。
...
> 注意：本リポジトリのコードとドキュメントには AI による生成・補助部分が含まれます。誤りが混在する可能性があります。

> **重要（v2.0.0）**: 本バージョンは **PyQt5 → PySide6 への完全移行** により、ライセンスが **LGPLv3** に変更されました。今後、本公開リポジトリではバイナリ配布のみ行い、ソースコードの公開は段階的に終了します（現時点では過渡期のためソースが残っていますが、将来的には削除予定）。利用方法は旧版（v1.x 系）とほぼ同様ですが、内部実装の変更により一部挙動が異なる場合があります。詳細は [RELEASE_NOTES_v2.0.0.md](docs/RELEASE_NOTES_v2.0.0.md) をご参照ください。

| バージョン  |    リリース日   | 主な更新内容                    |
| :-----: | :--------: | :------------------------ |
| v2.0.0 | 2025-11-08 | **メジャーアップデート**: PyQt5→PySide6完全移行・LGPLv3へライセンス変更・ソースコード非公開化（配布方針変更）・内部実装大幅刷新 |
| v1.20.0 | 2025-11-07 | データポータル統合機能完全実装・JSON書誌情報アップロード・画像一括アップロード・修正機能・ステータス管理 |

過去のバージョン履歴は [docs/archive/](docs/archive/) をご参照ください。
